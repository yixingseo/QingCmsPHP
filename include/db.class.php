...
{
    //构造函数
    function __construct($host,$user,$pass,$database,$port='3306',$charset='UTF8'){
        $opt = array();
        if($host){//mysql
            ...
            //var_dump(realpath($database));
            $dsn = 'odbc:driver={microsoft access driver (*.mdb)};dbq='.realpath($database);
        }
        try {
    ...
    //单行
    function fetchRow($sql,$parameters=array(),$type=PDO::FETCH_ASSOC){
        $rs = $this->prepare($sql);
        $rs->execute($parameters);
        $row = $rs->fetch($type);
        $rs->closeCursor();
    ...
    //单个字段
    function fetchValue($sql,$parameters = array(),$type=PDO::FETCH_ASSOC){
        $rs = $this->prepare($sql);
        $rs->execute($parameters);
        $row = $rs->fetchColumn();
        $rs->closeCursor();
    ...
    //更新
    function update($sql,$parameters){
        $rs = $this->prepare($sql);
        $rs->execute($parameters);
        return $rs->rowCount();
    }
